...
public class MsgInterceptor implements Interceptor {
    ...
    public void configServer(Controller c, String token) {
        ...
        System.out.println("configServer--echostr="+echostr);
        System.out.println("configServer--signature="+signature);
        System.out.println("configServer--token="+token);
        boolean isOk = SignatureCheckKit.me.checkSignature(signature, token, timestamp, nonce);
     
        System.out.println("configServer--isOk="+isOk);
        isOk=true;
        if (isOk) {
